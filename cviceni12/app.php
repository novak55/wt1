<?php
    ...
    if($_POST["login"] and $_POST["pwd"]) {
        ...
        if($db->loginUser()){
            $security->setRole();
            header('location: app.php?alert=5');
        }else{
            $security->setRole();
            header('location: app.php?alert=6');
        }
    }
    ...
    if($_POST['nazev_kapely'] !== null
        ...
        $db->ulozitKapelu();
        header('location: app.php?alert=2');
    }
    switch ($_GET['akce'])
    {
        case 'pridatKapelu':
            if($security->getRole() == 'admin'){
                ...
                header('location: app.php?alert=4');
            }
        break;
        case 'upravitKapelu':
            if($security->getRole() == 'admin'){
        ...
        case 'smazatKapelu':
            if($security->getRole() == 'admin'){
                ...
                header('location: app.php?alert=1');
            }else{
                header('location: app.php?alert=4');
            }
        break;
        ...
        case 'logout':
            $_SESSION = array();
            session_destroy();
            header('location: app.php?alert=3');
        break;
        case 'oblibit':
            $db->setOblibit();
            header('location: app.php?alert=7');
            break;
        case 'odlibit':
            $db->setOdlibit();
            header('location: app.php?alert=8');
            break;
        case 'hledej':
    ...
    SWITCH ($alerts){
        ...
        case 6:
            $alert['type'] = 'danger';
            $alert['text'] = 'Tento pokus o přihlášení nebyl úspěšný!';
        break;
        case 7:
            $alert['type'] = 'success';
